...
| 10 | `HealthEndpointTest` "com as anotações de `PipelineFlowTest`" | Sem `@AutoConfigureMockMvc`, o MockMvc montado à mão não corre o Spring Security, e o `show-details: when-authorized` do health esconde os detalhes (resposta só `{"status":"UP"}`). O `AutoConfigureMockMvc` criaria contexto novo (LocalStack novo — o risco do plano proíbe). Solução: juntar o `springSecurityFilterChain` como filter explícito ao MockMvc manual — segurança real, contexto reutilizado | Detalhe de execução |
| 10 | `TextractExtractor.status()` lê a região | Via `ExtractionProperties` injetado no construtor (o `SdkServiceClientConfiguration` não expõe a região sem um cast a `AwsServiceClientConfiguration`) | Detalhe de execução |
| 4 | `CorrelationPropagationTest` (Teste A) | O upload passou a usar um conteúdo binário próprio: o `PipelineFlowTest` submete o mesmo bytes e a regra de duplicados por hash binário marcava o segundo documento como `NEEDS_REVIEW` quando os dois partilham a BD (contexto em cache). O teste da correlação não depende do conteúdo | Detalhe de execução |
| 1 e 6 | Testes que usavam `/actuator/health` como "endpoint público 200" | Com os health checks do passo 10, o `/actuator/health` reflete as dependências e deixa de ser garantidamente 200 em contextos sem LocalStack (S3/SQS em baixo → 503). `CorrelationIdFilterTest` passou a usar `/actuator/info`; `ActuatorSecurityTest.healthIsPublic` afirma "não 401/403"; `ApplicationContextTest` aceita UP ou DOWN; `HealthEndpointTest` ganhou o caso "health público sem token" (com LocalStack, 200) | Detalhe de execução |
| | | | |
