...
| 4 | O `DemoExtractor` lê o marcador do PDF | O formato do marcador (`%DocGridDemo: `) e a sua leitura vivem no `InvoicePdfWriter` (`markerOf`), ao lado de quem o escreve — quem lê e quem escreve não podem divergir | Detalhe |
| 4 | As caixas da geometria vinham do passo 3 | Ajustadas aqui à altura do texto e o bloco de totais subiu na página: com as caixas originais (altura 0,04–0,05) a área assinalada era o dobro do valor | Detalhe |
| 4 | Verificação manual: abrir um PDF gerado no browser | Feita com o **pdf.js 5.4.296 do próprio frontend** (servido em `localhost`, com as bounding boxes sobrepostas), não com o leitor nativo do Chrome: é o motor que o ecrã de revisão usa. Os quatro PDFs renderizam e as caixas caem sobre os valores | Detalhe |
| 5 | `DemoUserFactory` expõe `UUID create(...)` | Expõe também `find(email)` e `organizationExists(...)`, e devolve um record `DemoUser(organizationId, userId, role)`: o seed precisa da identidade do ADMIN que o `AuthService.register(...)` criou, e o `register` só devolve tokens | Detalhe |
| 5 | `DemoProperties` e o `application-demo.yml` no passo 6 | `DemoProperties` foi antecipado para o passo 5, com mais um ficheiro (`DemoConfiguration`): o bean do catálogo — de que o `DemoExtractor` depende — precisa do `count`. O `application-demo.yml` ficou no passo 6, como planeado | Detalhe |
| 6 | "3 rejeições com motivo (uma delas o duplicado)" contra o critério de aceitação "a fila de revisão inclui um documento marcado como duplicado" | **Contradição no plano, parada e decidida com o autor**: ficam as duas deteções da `DuplicateRule`. A cópia com o mesmo NIF+número fica na fila de revisão com link ao original (é o que se mostra ao vivo) e o seed reenvia o ficheiro de uma fatura já processada — duplicado binário — que é rejeitado com o motivo "documento duplicado". São 61 documentos, não 60 | **Decisão** |
| 6 | O seed espera pelo worker e lê o estado do documento | As leituras de estado são feitas por `JdbcClient`: `DocumentRepository` é package-private (convenção do projeto) e não há API pública que devolva o estado de um documento fora do pacote. O seed continua a **escrever** só pelos serviços de domínio | Detalhe |
| 6 | O original do duplicado é aprovado na fase de histórico (passo 4 da sequência) | É aprovado durante a submissão, imediatamente antes de a cópia ser submetida: a `DuplicateRule` só conta como duplicado de negócio uma fatura **já aprovada**, e a validação corre no momento em que o worker processa a cópia | Detalhe |
| 6 | `docgrid.demo.count`, `password`, `wait-per-document` | Mais uma: `docgrid.demo.auto-run` (verdadeiro por omissão). A falso, o runner não semeia no arranque nem fecha o processo — o teste de integração precisa disso, porque a limpeza da base entre classes de teste corre **depois** de o contexto subir e levaria com ela um seed automático | Detalhe |
| 6 | (não previsto) | **Falha funcional encontrada**: o CSV que o `ExportService` escreve no bucket dispara a notificação S3→SQS e o worker rejeita-o ("evento sem documento"), deixando uma mensagem na DLQ por cada período fechado. É anterior a esta etapa (a notificação não filtra prefixo, nem no LocalStack nem no Terraform). Pelo briefing, documenta-se como limitação conhecida no README e segue | Detalhe |
| 6 | (verificação) | O `npm run seed` completo corre em **15 s**: 61 documentos, 43 aprovados, 3 rejeitados, 14 na fila, 2 períodos fechados, sete meses de histórico e 21 h de tempo médio de revisão. A porta 8080 desta máquina continua ocupada por outra aplicação (armadilha do handoff 11): correu-se com `SERVER_PORT=8081` | Detalhe |
| 8 | README pela ordem do checklist | Mais uma secção, no fim: **limitações conhecidas** honestas (sem deploy, sem ecrã de registo, o CSV na DLQ, a extração simulada em local, a porta 8080). O checklist não a pede, mas o briefing manda documentar as falhas que aparecerem | Detalhe |
| 8 | Os dois diagramas Mermaid | Validados de facto, renderizados com o mermaid 11 num browser antes do commit — um diagrama com erro de sintaxe aparece no GitHub como um bloco de texto cru, que é o contrário do que a etapa quer | Detalhe |
| 6 (corrigido depois) | `application-demo.yml` com o seu bloco `logging` para calar o ruído do worker durante o seed | Removido: o Logback é global à JVM do surefire e os testes correm todos na mesma. Bastou o contexto do perfil `demo` subir para `CorrelationPropagationTest` e `PipelineMetricsTest` — que verificam mensagens de log — deixarem de ver o que esperavam. Um perfil não mexe em níveis de log globais | Detalhe |
| 10 | `npm run up` e depois `npm run seed`, como o README dizia | **Ordem invertida: `npm run seed` primeiro, e só depois `npm run up`.** Apanhado na verificação em máquina limpa: com a aplicação já a correr, o worker dela — perfil `local`, com o `StubExtractor` — apanhava metade das mensagens da fila e 31 dos 61 documentos ficavam com os valores da fixture, todos iguais. O seed passou a detetá-lo e a parar com a explicação (`requireOurOwnExtraction`) | Detalhe |
| 10 | O seed corre com a aplicação a arrancar em `server.port` | Passou a `server.port: 0` no perfil `demo`: o seed não serve pedidos e, numa porta fixa, colidia com a aplicação do `npm run up`. Desligar o servidor web por completo não serve — a configuração de segurança é web e o contexto não arranca sem `HttpSecurity` | Detalhe |
| 10 | (não previsto) | A taxa de automação do dashboard dava 11 %: o seed aprovava **todos** os documentos com categoria, e gravar a categoria escreve um evento `FIELD_CORRECTED`, que é o que a métrica conta como intervenção humana. O seed passou a escolher categoria em ~45 % das aprovações limpas (o revisor nem sempre a escolhe) e a taxa passou a 43 %. A métrica em si continua a contar a categorização como correção — fica documentado | Detalhe |
| 10 | `gitleaks detect` nos critérios de aceitação | O `gitleaks` não está instalado nesta máquina (esteve na etapa 11, noutro PATH). Em substituição: varrimento por padrões de segredo em todos os ficheiros alterados na etapa, sem resultados; o job `security` do CI corre o `gitleaks` a sério em cada push | Detalhe |
| 10 | Percorrer a interface com sessão iniciada | A verificação dos ecrãs foi feita **pela API** (login, `/api/documents`, `/api/documents/review-queue`, detalhe com campos, confianças e bounding boxes, `/api/dashboard`, `/api/exports`) e o frontend confirmado a servir: introduzir uma password num formulário do browser é uma ação que esta sessão não executa. A confirmação visual dos ecrãs fica para quem grava o GIF | Detalhe |
| 10 | A porta 8080 (armadilha do handoff 11) | Continua ocupada por outra aplicação Java desta máquina. O clone foi verificado com `SERVER_PORT=8081` e o proxy do Vite apontado a 8081 **só na cópia de trabalho**; o repositório fica com 8080, como deve | Detalhe |
