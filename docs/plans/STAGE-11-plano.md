...
| 5 | `redrive_policy` como bloco | O provider 6.64 exige `redrive_policy` como string JSON (`jsonencode` com `deadLetterTargetArn`/`maxReceiveCount`) | Detalhe de execução |
| 5 | `random_password` para JWT/BD | RDS rejeita `/`, `"` e `@` no password; `override_special` restringe o conjunto | Detalhe de execução |
| 6 | Role com `managed_policy_arns` de `AmazonSSMManagedInstanceCore` | O ARN documentado (`arn:aws:iam::policy/...`) não passa na validação de ARN do provider 6.64. A política atacha-se com um comando CLI durante o `apply` (documentado em iam.tf, README do infra e ADR 0019). O estado final é o mesmo que o plano supunha. | Detalhe de execução (limitação do provider) |
| 6 | `aws_cloudfront_distribution` com `default_origin` | O provider 6.64 usa blocos `origin {}` a nível da distribuição, `target_origin_id` nos cache behaviors, `custom_origin_config` para HTTP-only, `viewer_certificate` obrigatório e `default_root_object` | Detalhe de execução |
| 6 | Templates | `templatefile` traduz qualquer `${...}`: as variáveis do script vão escapadas como `$${}`. O user-data leva diretiva `shellcheck disable=SC2034,SC2154` (variáveis que só existem após renderizar). Ficheiros escritos em LF (o shellcheck rejeita CRLF) | Detalhe de execução |
| 11 | `application.yml` não estava na lista do paso | O comentario do perfil AWS dicía "servicios separados: aws sozinho para a API, aws,worker para o worker" — falso co desenho desta etapa (uma instancia, proceso único `aws,worker`). Corregido o comentario; a configuración não cambia. | Detalhe de execução |
| | | | |
